...
push n (VMState p s m) =  (VMState p (n : s) m)
\end{code}
...
lookMemory n (VMState p s m) = (VMState p s m)
   where
     s = (snd $ (filter (search n) m) !! 0) : s
     search n touple 
        | fst(touple) == n = True
        | otherwise = False
\end{code}
...
searchAndSetMemory :: Name -> VMState -> [(Name,Int)]
searchAndSetMemory _ (VMState _ _ []) = []
searchAndSetMemory n (VMState p s (x:xs)) 
    | fst(x) == n = buildList s n x (x:xs)
    | otherwise = searchAndSetMemory n (VMState p s xs) 
    where 
      buildList st name touple memo = (takeWhile(/= touple) memo) ++ [(name,(st !! 0))] ++ (dropWhile(== touple) memo)

setMemory :: Name -> VMState -> VMState
setMemory n (VMState p s m) = (VMState p s m)
   where
     m = searchAndSetMemory n (VMState p s m)
